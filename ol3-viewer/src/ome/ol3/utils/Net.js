	...
	try {
			...
			if (len > 0) {
				for (var p=len-1;p>=0;p--)
					if (uri.charAt(p) !== '/') break; // we stop, if the last one is not a '/'
...
ome.ol3.utils.Net.checkAndSanitizeServerAddress = function(addressOrIp) {
	...
	try {
			...
			if (len > 0) {
				for (var p=len-1;p>0;p--) // the first one could be a '/', e.g. file protocol
					if (addressOrIp.charAt(p) !== '/') break; // we stop, if the last one is not a '/'
...
ome.ol3.utils.Net.isSameOrigin = function(server) {
	...
	if (window.location.protocol === 'file:')
		...
		browserServerInformation = window.location.protocol + "//" + window.location.host;
...
ome.ol3.utils.Net.checkRequestParameters = function(
	is_jsonp_request, server, uri,  success, error, method, headers, timeout, content) {
